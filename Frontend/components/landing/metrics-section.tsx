...
const metrics = [
  { 
    value: 420890, 
    suffix: " USD", 
    label: "Total value encrypted",
    sublabel: "+8.4% from yesterday"
  },
  { 
    value: 100, 
    suffix: "%", 
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="mt-12 p-6 rounded-xl bg-card border border-border card-shadow">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="font-mono text-sm text-muted-foreground">Protocol activity feed</span>
          </div>
          <div className="font-mono text-xs space-y-2 text-muted-foreground overflow-hidden h-24">
            <ActivityLine time="now" event="FHE_DEPOSIT [encrypted]" region="sepolia" status="Confirmed" latency="12ms" />
            <ActivityLine time="1s" event="GATEWAY_CALLBACK_FULFILL" region="zama" status="Success" latency="18ms" />
            <ActivityLine time="2s" event="FHE_BORROW [encrypted]" region="sepolia" status="Confirmed" latency="45ms" />
            <ActivityLine time="3s" event="HEALTH_CHECK_REQUEST" region="zama" status="Pending" latency="12ms" />
          </div>
        </div>
      </div>
    </section>
